...
#define roundUp(val, mod) \
	(((val) + ((mod) - 1) * ((val) > 0)) / (mod) * (mod))

#define roundDown(val, mod) \
	(((val) - ((mod) - 1) * ((val) < 0)) / (mod) * (mod))
...
void le2d_step_all_simple(le_task *t, int steps) {
	const real k1x = t->dt * t->mat.c1 / t->h.x;
	const real k2x = t->dt * t->mat.c2 / t->h.x;
	const real k1y = t->dt * t->mat.c1 / t->h.y;
	const real k2y = t->dt * t->mat.c2 / t->h.y;
	for (int step = 0; step < 2 * steps; ++step) {
		if (step % 2 == 0) {
			for (int j1 = 0; j1 < t->n.y; j1++) {
				for (int i1 = 0; i1 < t->n.x; i1++) {
							le_w d;
							le_w w_2, w_1, w, w1, w2;
...
{
	//le2d_step_all_simple(t, steps); return;
