...
void le_split_grid(le_task* task, int thread_num)
{
...
void le_combine_grid(le_task* task, int thread_num)
{
...
void le_step_x(le_task *t, int thread_num)
{
...
void le_step_y(le_task *t, int thread_num)
{
...
void *le_step(void *data_v)
{
	st_pthread* data = (st_pthread*) data_v;
	le_task* task = data->task;
	int thread_num = data->thread_num;
	int i;
	le_split_grid(task, thread_num);
	pthread_barrier_wait(&b);
	for (i = 0; i < task->steps; i++) {
		le_step_x(task, thread_num);
		pthread_barrier_wait(&b);
		le_step_y(task, thread_num);
		pthread_barrier_wait(&b);
	}
	pthread_barrier_wait(&b);
	le_combine_grid(task, thread_num);
	pthread_exit(NULL);
}
