...
double le_step(le_task *task, int steps)
{
	...
	checkCudaErrors(cudaMalloc(&d_grid1, grid_size));
	checkCudaErrors(cudaMalloc(&d_grid2, grid_size));
	checkCudaErrors(cudaMemcpy(d_grid1, task->grid, grid_size, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_grid2, task->grid, grid_size, cudaMemcpyHostToDevice));

	cudaDeviceSynchronize();
	t = timer();
	// run kernel
	for (int i = 0; i < steps; i++) {
		le_step_x<<<blocksPerGridX, threadsPerBlockX, sharedMemSizeX>>>(nx, ny, k1x, k2x, task->mat, d_grid1, d_grid2);
		checkCudaErrors( cudaPeekAtLastError() );
		le_step_y<<<blocksPerGridY, threadsPerBlockY, sharedMemSizeY>>>(nx, ny, k1y, k2y, task->mat, d_grid2, d_grid1);
		checkCudaErrors( cudaPeekAtLastError() );
	}
	...
	// return data to host
	checkCudaErrors(cudaMemcpy(task->grid, d_grid1, grid_size, cudaMemcpyDeviceToHost));
	cudaFree(d_grid1);
	cudaFree(d_grid2);
