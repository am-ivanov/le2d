...
// WARNING SOA is not working with mpi
#ifdef USE_SOA
	...
	#define inds_vx(i, j) (*(pgrid + ind(i, j) + 0 * nx * parallel_width))
	#define inds_vy(i, j) (*(pgrid + ind(i, j) + 1 * nx * parallel_width))
	#define inds_sxx(i, j) (*(pgrid + ind(i, j) + 2 * nx * parallel_width))
	#define inds_sxy(i, j) (*(pgrid + ind(i, j) + 3 * nx * parallel_width))
	#define inds_syy(i, j) (*(pgrid + ind(i, j) + 4 * nx * parallel_width))
#else /* AOS */
	...
	#define inds_vx(i, j) (*(pgrid + ind(i, j) * NODE_SIZE + 0))
	#define inds_vy(i, j) (*(pgrid + ind(i, j) * NODE_SIZE + 1))
	#define inds_sxx(i, j) (*(pgrid + ind(i, j) * NODE_SIZE + 2))
	#define inds_sxy(i, j) (*(pgrid + ind(i, j) * NODE_SIZE + 3))
	#define inds_syy(i, j) (*(pgrid + ind(i, j) * NODE_SIZE + 4))
#endif
...
typedef struct {
	...
	/* Number of nodes in pgrid on each axis. */
	le_point2 pn;
	
	/* additional memory for le_step */
	real *w;
} le_task;
