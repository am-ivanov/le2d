		...
		for (int j = 0; j < deviceCount; j++)
		{
			...
			if (j == 0) {
				for (int k = 0; k < NODE_SIZE; k++) {
					checkCudaErrors(cudaMemcpyPeer(d_bot[j] + k * nx * 2, j, d_grid2[j] + k * nx * dev_ny, j, sizeof(real) * nx));
					checkCudaErrors(cudaMemcpyPeer(d_bot[j] + k * nx * 2 + nx, j, d_grid2[j] + k * nx * dev_ny, j, sizeof(real) * nx));
				}
			} else {
				for (int k = 0; k < NODE_SIZE; k++) {
			...
			if (j == deviceCount - 1) {
				for (int k = 0; k < NODE_SIZE; k++) {
					checkCudaErrors(cudaMemcpyPeer(d_top[j] + k * nx * 2, j, d_grid2[j] + k * nx * dev_ny + nx * (dev_ny - 1), j, sizeof(real) * nx));
					checkCudaErrors(cudaMemcpyPeer(d_top[j] + k * nx * 2 + nx, j, d_grid2[j] + k * nx * dev_ny + nx * (dev_ny - 1), j, sizeof(real) * nx));
				}
			} else {
				for (int k = 0; k < NODE_SIZE; k++) {
