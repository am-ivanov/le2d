		...
		const real* __restrict__ in_grid, real* __restrict__ grid)
{
	...
	s_ind_vx(li, lj) = g_ind(in_grid, i, j, nx, ny, 0);
	s_ind_vy(li, lj) = g_ind(in_grid, i, j, nx, ny, 1);
	s_ind_sxx(li, lj) = g_ind(in_grid, i, j, nx, ny, 2);
	s_ind_sxy(li, lj) = g_ind(in_grid, i, j, nx, ny, 3);
	s_ind_syy(li, lj) = g_ind(in_grid, i, j, nx, ny, 4);
	if (li < 2) {
		s_ind_vx(li - 2, lj) = g_ind(in_grid, i - 2, j, nx, ny, 0);
		s_ind_vy(li - 2, lj) = g_ind(in_grid, i - 2, j, nx, ny, 1);
		s_ind_sxx(li - 2, lj) = g_ind(in_grid, i - 2, j, nx, ny, 2);
		s_ind_sxy(li - 2, lj) = g_ind(in_grid, i - 2, j, nx, ny, 3);
		s_ind_syy(li - 2, lj) = g_ind(in_grid, i - 2, j, nx, ny, 4);
	...
	omega_x(&mat, s_ind_vx(li-2, lj), s_ind_vy(li-2, lj), s_ind_sxx(li-2, lj), s_ind_sxy(li-2, lj), &w_2);
	omega_x(&mat, s_ind_vx(li-1, lj), s_ind_vy(li-1, lj), s_ind_sxx(li-1, lj), s_ind_sxy(li-1, lj), &w_1);
	omega_x(&mat, s_ind_vx(li, lj), s_ind_vy(li, lj), s_ind_sxx(li, lj), s_ind_sxy(li, lj), &w);
	omega_x(&mat, s_ind_vx(li+1, lj), s_ind_vy(li+1, lj), s_ind_sxx(li+1, lj), s_ind_sxy(li+1, lj), &w1);
	omega_x(&mat, s_ind_vx(li+2, lj), s_ind_vy(li+2, lj), s_ind_sxx(li+2, lj), s_ind_sxy(li+2, lj), &w2);

	reconstruct(w_2, w_1, w, w1, w2, k1, k2, &d);
	inc_x(&mat, s_ind_vx(li,lj), s_ind_vy(li,lj), s_ind_sxx(li,lj), s_ind_sxy(li,lj), s_ind_syy(li,lj), &vx, &vy, &sxx, &sxy, &syy, &d);
		...
		const real* __restrict__ in_grid, real* __restrict__ grid)
{
	...
	s_ind_syy(li, lj) = g_ind(in_grid, i, j, nx, ny, 4);
	if (lj < 2) {
		s_ind_vx(li, lj - 2) = g_ind(in_grid, i, j - 2, nx, ny, 0);
		s_ind_vy(li, lj - 2) = g_ind(in_grid, i, j - 2, nx, ny, 1);
		s_ind_sxx(li, lj - 2) = g_ind(in_grid, i, j - 2, nx, ny, 2);
		s_ind_sxy(li, lj - 2) = g_ind(in_grid, i, j - 2, nx, ny, 3);
		s_ind_syy(li, lj - 2) = g_ind(in_grid, i, j - 2, nx, ny, 4);
	...
	omega_y(&mat, s_ind_vx(li, lj-2), s_ind_vy(li, lj-2), s_ind_sxy(li, lj-2), s_ind_syy(li, lj-2), &w_2);
	omega_y(&mat, s_ind_vx(li, lj-1), s_ind_vy(li, lj-1), s_ind_sxy(li, lj-1), s_ind_syy(li, lj-1), &w_1);
	omega_y(&mat, s_ind_vx(li, lj), s_ind_vy(li, lj), s_ind_sxy(li, lj), s_ind_syy(li, lj), &w);
	omega_y(&mat, s_ind_vx(li, lj+1), s_ind_vy(li, lj+1), s_ind_sxy(li, lj+1), s_ind_syy(li, lj+1), &w1);
	omega_y(&mat, s_ind_vx(li, lj+2), s_ind_vy(li, lj+2), s_ind_sxy(li, lj+2), s_ind_syy(li, lj+2), &w2);

	reconstruct(w_2, w_1, w, w1, w2, k1, k2, &d);
	inc_y(&mat, s_ind_vx(li,lj), s_ind_vy(li,lj), s_ind_sxx(li,lj), s_ind_sxy(li,lj), s_ind_syy(li,lj), &vx, &vy, &sxx, &sxy, &syy, &d);
...
double le_step(le_task *task, int steps)
{
	...
	for (int i = 0; i < steps; i++) {
		le_step_x<<<blocksPerGrid, threadsPerBlock, sharedMemSize>>>(nx, ny, k1x, k2x, task->mat, d_grid1, d_grid2);
		gpuErrchk( cudaPeekAtLastError() );
		le_step_y<<<blocksPerGrid, threadsPerBlock, sharedMemSize>>>(nx, ny, k1y, k2y, task->mat, d_grid2, d_grid1);
		gpuErrchk( cudaPeekAtLastError() );
	}
