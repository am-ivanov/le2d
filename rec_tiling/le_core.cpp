 ...
 */
#define ind(i, j) ((i + 2) + (j + 2) * (t->n.x + 4))
#define gind(i, j) (((le_node*)t->grid)[ind((i), (j))])
#define gind2(i, j) (((le_node*)t->grid2)[ind((i), (j))])
#define wind(i, j) (((le_w*)t->w)[ind((i), (j))])
#define wind2(i, j) (((le_w*)t->w2)[ind((i), (j))])
...
inline real le_fmin(real a, real b) { return a > b ? b : a; }
inline real le_fmax(real a, real b) { return a > b ? a : b; }
inline real le_fmax3(real a, real b, real c) { return le_fmax(a, le_fmax(b, c)); }


#define limiter_fminmod(r) (le_fmax(0.0, le_fmin(1.0, (r))))
#define limiter_cir(r) (0.0)
#define limiter_superbee(r) (le_fmax3(0.0, le_fmin(1.0, 2.0 * r), le_fmin(2.0, r)))
...
/*
 * Second order TVD scheme (http://en.wikipedia.org/wiki/Total_variation_difminishing).
 */
...
void le_init_task(le_task *task, const real dt, const le_vec2 h, const le_material mat, const le_point2 n, const int stype)
{
	...
	task->mat = mat;
	task->n   = n;
	task->grid = (real*)sse_malloc(sizeof(le_node) * (n.x + 4) * (n.y + 4));
	task->grid2 = (real*)sse_malloc(sizeof(le_node) * (n.x + 4) * (n.y + 4));
	task->stype = stype;
...
inline void inc_x2(const le_material *m, le_node *pn, le_node *n, const le_w *d)
{
	const real d1 = 0.5 * d->w1;
	const real d2 = 0.5 * d->w2;
	const real d3 = 0.5 * d->w3;
	...
	n->s.xx = pn->s.xx + (d2 - d1) * m->rhoc1;
	n->s.yy = pn->s.yy + (d2 - d1) * m->rhoc3;
	n->s.xy = pn->s.xy + m->rhoc2 * (d4 - d3);
}

inline void inc_y2(const le_material *m, le_node *pn, le_node *n, const le_w *d)
{
	const real d1 = 0.5 * d->w1;
	const real d2 = 0.5 * d->w2;
	const real d3 = 0.5 * d->w3;
	...
	n->s.yy = pn->s.yy + (d2 - d1) * m->rhoc1;
	n->s.xx = pn->s.xx + (d2 - d1) * m->rhoc3;
	n->s.xy = pn->s.xy + m->rhoc2 * (d4 - d3);
}
...
void le_step_all3(le_task *t, int steps, int* indX, int* indY, int* indT)
{
	const real k1x = t->dt * t->mat.c1 / t->h.x;
	const real k2x = t->dt * t->mat.c2 / t->h.x;
	const real k1y = t->dt * t->mat.c1 / t->h.y;
	const real k2y = t->dt * t->mat.c2 / t->h.y;

	for (int k = 0; k < t->n.x * t->n.y * steps * 2; ++k) {
		int step = indT[k];
		int i = indX[k];
		int j = indY[k];
		le_w d;
		le_w w_2, w_1, w, w1, w2;
		if (step % 2 == 0) {
			omega_x(&t->mat, &gind(i-2, j), &w_2);
			omega_x(&t->mat, &gind(i-1, j), &w_1);
			omega_x(&t->mat, &gind(i+0, j), &w);
			omega_x(&t->mat, &gind(i+1, j), &w1);
			omega_x(&t->mat, &gind(i+2, j), &w2);
			reconstruct(w_2, w_1, w, w1, w2, k1x, k2x, &d);
			inc_x2(&t->mat, &gind(i, j), &gind2(i, j), &d);
		} else {
			omega_y(&t->mat, &gind2(i, j-2), &w_2);
			omega_y(&t->mat, &gind2(i, j-1), &w_1);
			omega_y(&t->mat, &gind2(i, j+0), &w);
			omega_y(&t->mat, &gind2(i, j+1), &w1);
			omega_y(&t->mat, &gind2(i, j+2), &w2);
			reconstruct(w_2, w_1, w, w1, w2, k1y, k2y, &d);
			inc_y2(&t->mat, &gind2(i, j), &gind(i, j), &d);
		}
	}
}

void le_step_all2(le_task *t, int steps)
{
	const real k1x = t->dt * t->mat.c1 / t->h.x;
	const real k2x = t->dt * t->mat.c2 / t->h.x;
	const real k1y = t->dt * t->mat.c1 / t->h.y;
...
void le_step_rec(le_task *t, int steps, CachedGen& gen)
{
	const real k1x = t->dt * t->mat.c1 / t->h.x;
	const real k2x = t->dt * t->mat.c2 / t->h.x;
	const real k1y = t->dt * t->mat.c1 / t->h.y;
...
void le_step_all(le_task *t, int steps)
{
	int Lx = 0;
	int Ux = t->n.x / 2;
	int Ly = 0;
							...
							if (tt % 2 == 0) {
								{
										...
										le_w d;
										le_w w_2, w_1, w, w1, w2;
										omega_x(&t->mat, &gind(i1-2, j1), &w_2);
										omega_x(&t->mat, &gind(i1-1, j1), &w_1);
										omega_x(&t->mat, &gind(i1+0, j1), &w);
										omega_x(&t->mat, &gind(i1+1, j1), &w1);
										omega_x(&t->mat, &gind(i1+2, j1), &w2);
										reconstruct(w_2, w_1, w, w1, w2, k1x, k2x, &d);
										inc_x2(&t->mat, &gind(i1, j1), &gind2(i1, j1), &d);
								}
							...
							} else {
								{
										...
										le_w d;
										le_w w_2, w_1, w, w1, w2;
										omega_y(&t->mat, &gind2(i1, j1-2), &w_2);
										omega_y(&t->mat, &gind2(i1, j1-1), &w_1);
										omega_y(&t->mat, &gind2(i1, j1+0), &w);
										omega_y(&t->mat, &gind2(i1, j1+1), &w1);
										omega_y(&t->mat, &gind2(i1, j1+2), &w2);
										reconstruct(w_2, w_1, w, w1, w2, k1y, k2y, &d);
										inc_y2(&t->mat, &gind2(i1, j1), &gind(i1, j1), &d);
								}
	...
	for (int step = 0; step < 2 * steps; ++step) {
		if (step % 2 == 0) {
			for (int j = 0; j < t->n.y / 2; j++) {
				for (int i = 0; i < t->n.x / 2; i++) {
